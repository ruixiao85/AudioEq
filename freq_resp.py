...
if __name__ == "__main__":
  ...
  # "Shure SE215"
  anchorHp="Sennheiser HD 800"; anchorHpType="onear"
  HpBrands=["akg","audio-technica","beyerdynamic","audeze","beats","dan","denon","etymotic","focal","fostex","hifiman","fiio","kz","mee","meze","monoprice","monster","oppo","philips","sennheiser","shure","sony","stax","ultimate","westone","tin"]
  if not os.path.isdir(anchorHp): # get cross-source readings based on the anchorHp
    os.makedirs(anchorHp,exist_ok=True)
    ...
    # for sourceDir in [ "oratory1990"]:
      rootpath=f"AutoEq/measurements/{sourceDir}/data"
      hpa=FreqResp.from_csv(f"{rootpath}/{anchorHpType}/{anchorHp}/{anchorHp}.csv")
      for myHpType in ["onear", "inear"]:
        for dirpath,dirnames,filenames in os.walk(f"{rootpath}/{myHpType}"):
          for dirname in dirnames:
            if dirname.split()[0].lower() in HpBrands:
              ...
              try:
                ...
                hpd.res_to_csv(f"{anchorHp}/{dirname}_{sourceDir}.csv")
              except: pass
  ...
  if True:
    ...
    "Audio-Technica AT897pos_oratory1990_res",
    "Audio-Technica ATH-W5000 2013_innerfidelity_res",
    "Audio-Technica ATH-W5000_innerfidelity_res",
    "Sennheiser HD 580_innerfidelity_res",
    "Sennheiser HD 600_oratory1990_res",
    "Sennheiser HD 800_innerfidelity_res",
    "Sennheiser HD 800_oratory1990_res",
    # "Sennheiser IE 800_oratory1990_res",
    "Tin HiFi T3_oratory1990_res"
    ]
    for hp in myHps:
      hpc=FreqResp.from_csv(f"{anchorHp}/{hp}.csv")
      os.makedirs(f"{anchorHp}/{hp}",exist_ok=True)
      for dirpath,dirnames,filenames in os.walk(f"{anchorHp}"):
        for filename in filenames:
          if filename.endswith(".csv") and filename!=f"{hp}.csv":
            # print(dirpath,filename)
            hpt=FreqResp.from_csv(f"{anchorHp}/{filename}")
            hpd=FreqResp.res_adjust(hpc,hpt) # diff
            for ratio in [0.6,1.0]:
              hpd.res_to_xgeq(f"{anchorHp}/{hp}/{filename}.csv",ratio)
